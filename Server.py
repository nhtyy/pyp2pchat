        ...
        while connected:
            ...
            if msg_length:  # IF LENGTH IS NOT 0
                ...
                if msg[0:2] == "!R":
                    ...
                    if room in self.ROOMS_MSG:
                        for value in self.ROOMS_MSG[room]:
                            try:
                                Utils.connsend(conn, "!K" + room)
                            except socket.error:
                                self.ROOM_MEMBER[room].remove(conn)
                                break
                            Utils.connsend(conn, value)
                    else:
                ...
                if msg[0:2] == "!K":  # ROuter COMMAND
                    ...
                    msg_text = conn.recv(int(msg_size)).decode(Server.FORMAT)

                    if self.server_addr != self.peer_addr:
                        if conn != self.CONNECTIONS[0]:
                            self.send_to_peer(msg)
                            self.send_to_peer(msg_text)

                    for x in self.ROOM_MEMBER[room]:
                        if x != conn:
                            try:
                                Utils.connsend(x, "!K" + room)
                            except socket.error:
                                self.ROOM_MEMBER[room].remove(x)
                                break
                            Utils.connsend(x, msg_text)

                    self.ROOMS_MSG[room].append(msg_text)

                if broadcast:  # send mesasge to EVERY room member
                    for x in self.ROOM_MEMBER[current_room_conn]:
                        Utils.connsend(x, "!K" + current_room_conn)
                        Utils.connsend(x, broadcast)
            ...
            if msg_length:
                ...
                if data[0:2] == "!K":
                    ...
                    if self.roomRequest is not None:
                        Utils.connsend(self.roomRequest, "!K" + room)
                        Utils.connsend(self.roomRequest, data)
                        self.roomRequest = None
                    else:
